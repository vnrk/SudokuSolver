...
//See if a number is the only number possible in a place
//[3,4,6][3,4][3.4] - only possible number at 1st position is 6
static int isThisUnique(int row, int col, int val) {
    int i, j, e_i, e_j;
    int isUnique = 1;
...
static void updatePossibleValue(int row, int col, int val) {
    int i, j, e_i, e_j;
    //Remove possible values in row and column.
    for (i = 0; i < 9; i++) {
...
int evaluate() {
    ...
    //if(!noOfUpdates)
    //	printf("Not even one update cant help :(\n");
    return noOfUpdates;
}
